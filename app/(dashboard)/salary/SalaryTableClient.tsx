...
import { salaryColumns } from "@/components/salary/SalaryColumns";
...
import { SalaryWithPerson, SalaryStatus, MONTHS } from "@/types";

const currentYear = new Date().getFullYear();
const YEARS = Array.from({ length: 5 }, (_, i) => currentYear - 2 + i);

export function SalaryTableClient({ salaries }: { salaries: SalaryWithPerson[] }) {
  ...
  const filtered = useMemo(() => {
    return salaries.filter((s) => {
      ...
      const matchMonth = monthFilter === "ALL" || s.month.toString() === monthFilter;
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-wrap gap-3">
        ...
        <Select
          value={statusFilter}
          onValueChange={(v) => setStatusFilter((v ?? "ALL") as SalaryStatus | "ALL")}
        >
      ...
      <DataTable columns={salaryColumns} data={filtered} />
    </div>
  );
}