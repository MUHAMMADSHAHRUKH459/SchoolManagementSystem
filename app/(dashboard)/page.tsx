  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
        <Card className="border border-green-100 shadow-sm overflow-hidden">
            ...
            <div className="flex items-stretch">
              ...
              <div className="flex-1 p-5">
                <div className="flex items-start justify-between">
                  <div>
                    <p className="text-[11px] font-semibold uppercase tracking-wider text-muted-foreground mb-1">Monthly Income</p>
                    <p className="text-3xl font-bold text-emerald-600">PKR {income.toLocaleString()}</p>
                    <p className="text-xs text-muted-foreground mt-1">Fee collections — {monthName}</p>
                  </div>
          ...
          <CardContent className="p-0">
            <div className="flex items-stretch">
              ...
              <div className="flex-1 p-5">
                <div className="flex items-start justify-between">
                  <div>
                    <p className="text-[11px] font-semibold uppercase tracking-wider text-muted-foreground mb-1">Monthly Expense</p>
                    <p className="text-3xl font-bold text-red-600">PKR {expense.toLocaleString()}</p>
                    <p className="text-xs text-muted-foreground mt-1">Salary payments — {monthName}</p>
                  </div>
        ...
        <Card className="border border-border shadow-sm">
          <CardHeader className="pb-3 border-b border-border bg-muted/20">
            <div className="flex items-center justify-between">
              ...
              <Badge variant="secondary" className="text-xs bg-green-100 text-green-700">{monthName}</Badge>
            </div>
          </CardHeader>
          <CardContent className="pt-3 space-y-1">
            {recentFees.length === 0 ? (
              <p className="text-sm text-muted-foreground py-6 text-center">No fee records this month</p>
            ) : (
              recentFees.map((fee: any) => (
                <div key={fee.id} className="flex items-center justify-between p-2.5 rounded-lg hover:bg-green-50/60 transition-all duration-150 group">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground">Class {fee.student.class}{fee.student.section}</p>
                    </div>
                  </div>
                  <div className="text-right">
                    <p className="text-sm font-semibold">PKR {Number(fee.totalAmount).toLocaleString()}</p>
                    <Badge
        ...
        <Card className="border border-border shadow-sm">
          <CardHeader className="pb-3 border-b border-border bg-muted/20">
            <div className="flex items-center justify-between">
              ...
              <Badge variant="secondary" className="text-xs bg-blue-100 text-blue-700">{monthName}</Badge>
            </div>
          </CardHeader>
          <CardContent className="pt-3 space-y-1">
            {recentSalaries.length === 0 ? (
              <p className="text-sm text-muted-foreground py-6 text-center">No salary records this month</p>
            ) : (
              recentSalaries.map((sal: any) => {
                const person = sal.teacher ?? sal.staff;
                return (
                  <div key={sal.id} className="flex items-center justify-between p-2.5 rounded-lg hover:bg-blue-50/60 transition-all duration-150 group">
                    <div className="flex items-center gap-3">
                      <div className="h-8 w-8 rounded-full flex items-center justify-center shrink-0 text-white text-xs font-bold shadow-sm"
                      ...
                      <div>
                        ...
                        <p className="text-xs text-muted-foreground">{sal.teacher ? "Teacher" : "Staff"}</p>
                      </div>
                    </div>
                    <div className="text-right">
                      <p className="text-sm font-semibold">PKR {Number(sal.netSalary).toLocaleString()}</p>
                      <Badge
